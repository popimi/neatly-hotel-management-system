...
function RequestRefund() {
  ...
  return (
    <main className="h-[calc(100dvh-48px)] lg:h-[calc(100dvh-100px)] flex flex-col items-center">
      <div className="flex flex-col p-5 w-full max-w-[1024px] justify-center">
        ...
        <section className="bg-green-700 flex flex-col">
          <div className="bg-green-600 m-5 p-5 text-white">
            ...
            <p className="text-green-400">
              Cancellation Date:{" "}
              {refundDetail ? refundDetail.formatted_updated_at : "Loading..."}
            </p>
          </div>
          <div className="flex flex-row justify-between border-t border-t-green-500 text-white p-5 m-5">
            <p>Total Refund</p>
            <p>
              THB {refundDetail ? formatNumber(details.amount) : "Loading..."}
            </p>
          </div>
        </section>
      </div>
      <section className="flex justify-center p-5">
        <button onClick={handleNavigate} className="button-primary">
          Back to Home
        </button>
      </section>
    </main>
