...
export const registerSchema = ({ apiUrl }) =>
  Yup.object({
    username: Yup.string()
          ...
          try {
            if (String(value).length > 2) {
              const response = await axios.get(
                `${apiUrl}/check-available?checkColumn=username&checkValue=${value.toLowerCase()}`
              );
    ...
    phoneNumber: Yup.string()
      .matches(/^(\+|0)/, "Phone number must start with + or 0")
    ...
    email: Yup.string()
      ...
      .test(
        ...
        async (value) => {
          ...
          try {
            const response = await axios.get(
              `${apiUrl}/check-available?checkColumn=email&checkValue=${value.toLowerCase()}`
            );
            return response.data.data;
          } catch (error) {
...
export const loginSchema = ({ apiUrl, type, setType }) =>
  Yup.object({
      ...
      .test(
        ...
        async (value) => {
          try {
            ...
            if (String(value).length > 2) {
              const response = await axios.get(
                `${apiUrl}/check-available?checkColumn=${type}&checkValue=${value.toLowerCase()}`
              );
              return !response.data.data;
            }
          } catch (error) {
